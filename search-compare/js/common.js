...
$(window).on('resize', function(){
	headRoom();
  //also, recheck any dropdowns that are open
  if($('.show').length){
    isElementInViewport();
  }
});//end on resize
...
function headRoom() {
  ...
  if($('header').css('position') === 'fixed') {
    $('#show-user-table').css({'top': $('header').height(), 'bottom': 'auto'});
  }else{
    $('#show-user-table').css({'bottom': '200px', 'top': 'auto'});
  }
}
...
function isElementInViewport(){
  ...
  var bounding = showDiv.getBoundingClientRect();
