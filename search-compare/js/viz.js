...
function revenueExpenditureStaff(res, viz_type) {
  ...
  var footnote2Y = addFootnote(footnoteY + 40, 'S = Suppressed for personally identifiable information');
  
  //without_mls = res.librarian_staff - res.mls_librarian_staff;
  ...
  if( is_suppressed ) {
    ...
    pieChart('Operating Expenditures', 40, leftMargin, chartY, [{name: 'Collection', value: res.total_collection_expenditures}, {name: 'Other', value: non_collection_expenditures}]);
  } else {
    pieChart('Operating Expenditures by Type', 40, leftMargin, chartY, [{name: 'Staff', value: res.total_staff_expenditures}, {name: 'Collection', value: res.total_collection_expenditures}, {name: 'Other', value: res.other_expenditures}]);
  }
  pieChart('Collection Expenditures by Type', 40, leftMargin, 360, [{name: 'Print', value: res.print_expenditures}, {name: 'Electronic', value: res.electronic_expenditures}, {name: 'Other', value: res.other_collection_expenditures}]);
  pieChart('Paid Staff by Type', 40, leftMargin, 540, [{name: 'Total Librarians', value: res.librarian_staff}, {name: 'All Other Paid Staff', value: res.other_staff}]);
  setFooter(res);
...
function programService(res, viz_type) {
  ...
  //var fontSize = 14;
  //doc.setFontSize(fontSize);
  pieChart('Programs Offered', 70, leftMargin, chartY, [{name: "Children's Programs", value: res.kids_programs}, {name: "Young Adult Programs", value: res.ya_programs}, {name: "All Other Programs", value: res.all_other_programs}]);
  pieChart('Program Attendance', 70, leftMargin, chart2Y, [{name: "Children's Program Attendance", value: res.kids_program_audience}, {name: "Young Adult Program Attendance", value: res.ya_program_audience}, {name: 'All Other Program Attendance', value: res.all_other_attendance}]);
  setFooter(res);
  savePdf(doc, res, title);
}
...
function collectionCirculation(res, viz_type) {
  ...
  pieChart('Collection Materials by Type', 70, leftMargin, chartY, [{name: "Books and Other Items", value: res.print_materials}, {name: "Physical Audio and Video", value: physical_av}, {name: "E-Books", value: res.ebooks}, {name: "Digital Audio and Video", value: downloadable_av}]);
  pieChart('Circulation', 70, leftMargin, chart2Y, [{name: "Circulation of Children's Materials", value: res.kids_circulation}, {name: "Circulation of All Other Materials", value: other_circulation}]);
  setFooter(res);
  savePdf(doc, res, title);
}
