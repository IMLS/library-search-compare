...
var index_name = 'libraries_fy18_total_branches'; // comparison data grid labels and field names  
...
comparisonData = [{
  ...
  headings: ['Name', 'Population of Legal Service Area (LSA)', 'Central libraries', 'Branch libraries', 'Bookmobiles', 'Interlibrary relationship code', 'Legal basis code', 'Administrative structure code', 'FSCS public library definition', 'Geographic code'],
...
function getFullData() {
  ...
  if ($('#list-results').is(':visible') === false) {
    ...
    var query = params_obj['query']; // Create array of refinementList (facet) statements
...
function setAddUserCompareHandler() {
  ...
  $('.user-compare-btn').on('click', function (evt) {
    ...
    $('[data-fscs=' + evt.target.dataset.fscs + ']').each(function (index) {
    ...
    searchCompare.fscs_arr = _.uniq(searchCompare.fscs_arr); //getUserComparisonData();
...
function displayUserComparisonGrid(content, comparisonSelect, el) {
  ...
  tableData['headings'] = _.map(_.find(comparisonData, {
    'name': comparisonSelect
  }).headings);
  tableData['headings'].unshift('');

  for (var h in content.hits) {
    res = content.hits[h]; //var tableRow = [ '<a data-name="' + res[ 'library_name' ] + '" data-fscs="' + res[ "fscs_id" ] + '" href="details.html?fscs_id=' + res["fscs_id"] + '">' + res["library_name"] + ' (' + res[ "fscs_id" ] + ')' + '</a>' ];
...
function startAppJs() {
  ...
  var search = instantsearch({
    ...
    appId: 'CDUMM9WVUG',
    apiKey: '3cc392a5d139bd9131e42a5abb75d4ee',
  ...
  (function () {
    ...
    XMLHttpRequest.prototype.open = function () {
      this.addEventListener('load', function () {
        ...
        if (typeof res.results !== "undefined") {// getFullData( res.results );
        }
      });
  ...
  */
  ...
  var updateSearchUI = function updateSearchUI() {
    ...
    changeLegalLabels(); // call second search to populate comparison table

    searchCompare.instantSearchURL = search.createURL();
    getFullData(); //if an input refinement is empty, display a message.
  ...
  search.on('error', updateSearchUI); // fill in data grid variable selector
  ...
  _.forEach(comparisonData, function (value, key) {
    $('#comparison-select, #user-comparison-select').append($('<option></option>').attr('value', value.name).text(value.display_name));
  }); //end forEach
  ...
  window.addEventListener("beforeunload", function (e) {
    var returnURL = window.location.href; //console.log('now returnURL = '+returnURL);
