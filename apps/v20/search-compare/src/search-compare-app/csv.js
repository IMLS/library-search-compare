define(["exports"], function (_exports) {
  ...
  /*
    ...
    Forked from https://github.com/okfn/csv.js/blob/master/csv.js
  
    The only change is the export statement for ES6 module compatibility.
  
  */

  /* global jQuery, _ */
  var CSV = {}; // Note that provision of jQuery is optional (it is **only** needed if you use fetch on a remote file)
  ...
  (function (my) {
          ...
          if (cur !== options.quotechar) {
            ...
            if (!inQuote) {
              ...
              if (s.charAt(i + 1) === options.quotechar) {
                field += options.quotechar; // Skip the next char
    ...
    my.normalizeLineTerminator = function (csvString, dialect) {
      dialect = dialect || {}; // Try to guess line terminator if it's not provided.
    ...
    my.serialize = function (dataToSerialize, dialect) {
      ...
      if (babelHelpers.instanceof(dataToSerialize, Array)) {
        a = dataToSerialize;
      } else {
  ...
  })(CSV); // backwards compatability for use in Recline
