...
(function (root, factory) {
  ...
  var util = {
    ...
    createElement: function createElement(a, b) {
      ...
      if (b && "object" == babelHelpers.typeof(b)) {
        var e;
    ...
    append: function append(p, e) {
      return p && e && p.appendChild(e);
    },
    on: function on(e, type, callback) {
    ...
    isDate: function isDate(str) {
      ...
      if (!/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(str)) return false; // Parse the date parts to integers
    ...
    getBoundingRect: function getBoundingRect(el) {
      var win = window;
      var doc = document;
      var body = doc.body;
    ...
    button: function button(c, p, t) {
      return util.createElement('li', {
        class: c,
        html: '<a href="#" data-page="' + p + '">' + t + '</a>'
      });
    },
    flush: function flush(el, ie) {
      if (babelHelpers.instanceof(el, NodeList)) {
  ...
  var build = function build() {
    ...
    util.addClass(this.table, 'dataTable-table'); // Paginator

    var w = util.createElement('div', {
      class: 'dataTable-pagination'
    });
    var paginator = util.createElement('ul');
    util.append(w, paginator); // Pager(s) placement
    ...
    this.rect = util.getBoundingRect(this.table); // Convert rows to array for processing

    this.rows = [].slice.call(this.body.rows); // Update

    this.update(); // Fixed height
  ...
  var setClassNames = function setClassNames() {
    var o = this.options;
  ...
  var paginate = function paginate() {
    var perPage = this.options.perPage,
        rows = !!this.searching ? this.searchData : this.rows; // Check for hidden columns
  ...
  var render = function render() {
    if (this.hasRows && this.totalPages) {
      ...
      var index = this.currentPage - 1,
          frag = util.createFragment(); // Check for hidden column headings
      ...
      this.head.innerHTML = "";
      this.head.appendChild(tr); // Check for hidden column cells
  ...
  var renderPager = function renderPager() {
    ...
    util.addClass(this.links[this.currentPage - 1], 'active'); // append the links

    util.each(pager, function (i, p) {
      util.removeClass(p, 'active');
      util.append(frag, p);
    });
    util.addClass(this.links[this.currentPage - 1], 'active'); // next button
  ...
  var truncate = function truncate(a, b, c, d) {
    d = d || 2;
    var j,
        e = 2 * d,
        f = b - d,
  ...
  function DataTable(table, options) {
    ...
    this.options = util.extend(defaults, options); // Checks
  ...
  DataTable.prototype.init = function (options) {
    ...
    var that = this;
    this.options = util.extend(this.options, options || {}); // IE detection
  ...
  DataTable.prototype.destroy = function () {
    var o = this.options; // Remove the sorters
    ...
    this.clear(f); // Remove the className

    util.removeClass(this.table, "dataTable-table"); // Remove the containers
  ...
  DataTable.prototype.fixColumns = function () {
    var cells,
        hd = false; // If we have headings we need only set the widths on them
    // otherwise we need a temp header and the widths need applying to all cells

    if (this.table.tHead && this.headings.length) {
      var headings = this.head.rows[0].cells; // Reset widths
  ...
  DataTable.prototype.search = function (query) {
    ...
    this.clear();
    util.each(this.rows, function (idx, row) {
      var inArray = util.includes(this.searchData, row); // https://github.com/Mobius1/Vanilla-DataTables/issues/12
  ...
  DataTable.prototype.sortColumn = function (column, direction, callback) {
    ...
    this.sorting = true; // Convert to zero-indexed
    ...
    util.each(rows, function (i, tr) {
      ...
      var num = content.replace(/(\$|\,|\s|%)/g, ""); // Parse date strings
    ...
    if (options.type) {
      ...
      if (rows.length) {
        if (options.type === "txt" || options.type === "csv") {
          ...
          str = "INSERT INTO `" + options.tableName + "` ("; // Convert table headings to column names
        ...
        if (options.download) {
          ...
          options.filename += "." + options.type; // Create a link to trigger the download
      ...
      if (options.type === "csv") {
        ...
        if (rows.length) {
          util.each(rows, function (i, row) {
            obj.rows[i] = []; // Split the rows into values
        ...
        var json = util.isJson(options.data); // Valid JSON string
