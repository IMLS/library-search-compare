...
function comparePdf(base_res, compare_res, viz_type) {
  ...
  setTitle(title); // setLibraryBlock(base_res, compare_res);
...
function setData(base, compare, viz_type) {
  ...
  printColumnHeaders([base.library_name, compare.library_name], currentY); //currentY = currentY + fontSize*1.3;
  ...
  fields.forEach(function (type, index) {
    ...
    */
    ...
    sections.forEach(function (section, index) {
      ...
      var lineHeight = fontSize + padding;
      doc.setFontSize(fontSize);
      doc.setTextColor(52, 113, 91); //if( index !== 0 ) {

      currentY = currentY + lineHeight + 7; //}

      doc.text(sectionTitle, col1X, currentY);
      currentY = currentY + 3; //currentY = currentY + fontSize/2;
      ...
      section[sectionTitle].forEach(function (line, index) {
        if (striped) {
          addStripe(col1X, currentY, lineHeight);
        }

        label = _.keys(line).join();
        data_field = line[label]; //console.log(compare[data_field]);
...
function setFooter(res) {
  var baseY = 710; //doc.setDrawColor(52, 113, 91);
